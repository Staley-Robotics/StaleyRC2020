...
lower_green = np.array([58,147,55])
upper_green = np.array([109,255,255]) # 101,255,255
        ...
        for cnt in cntsSorted:
            ...
            if (checkContours(cntArea, hullArea)):
                ...
                if(len(biggestCnts) < 13):
                    ...
                    center = (int(x), int(y))
                    # Rounds radius of enclosing circle
                    radius = int(radius)
                    ...
                    print("rx: " + str(rx) + ", ry: " + str(ry) + ", rw: " + str(rw) + ", rh: " + str(rh))
                    cv2.circle(image, center, radius, (23, 184, 80), 1)

                    # Appends important info to array
                    if [cx, cy, rotation, cnt] not in biggestCnts:
                        biggestCnts.append([cx, cy, rotation, cnt, cv2.contourArea(cnt)])


        # Sorts array based on coordinates (leftmost to rightmost) to make sure contours are adjacent
        #biggestCnts = sorted(biggestCnts, key=lambda x: cv2.contourArea(x))
        print("PRE PROCESSED")
        print(biggestCnts)
        biggestCnts = sorted(biggestCnts, key=sortCnts)
        print("POST PROCESS")
        print(biggestCnts)
        # Target Checking
        for i in  range(0,len(biggestCnts)):
            ...
            if pitch != 0:
                distanceToTarget =calculateDistance(HEIGHT_OF_CAMERA,HEIGHT_OF_TARGET,pitch)
            else:
                distanceToTarget = 0
...
if __name__ == "__main__":
    ...
    while True:
        ...
        #networkTable.getBoolean("TapeProcessing", False)
        if(True):
            print("processing for tape")
            #Lowers exposure to 0
            cap.autoExpose = False
            print("set exposure to 2")
...
#boolean TapeProcessing, number VideoTimestamp, number tapeYaw
